...
@RestController
@RequestMapping("/api/employee-cloud-project-kafka")
public class EmployeeCloudProjectKafkaResource {

    private final Logger log = LoggerFactory.getLogger(EmployeeCloudProjectKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public EmployeeCloudProjectKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
